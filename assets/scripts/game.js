    ...
    constructor(levelPlan) { // class constructor, which create the level plan in the form of characters comprehensible by the js file
        ...
        this.levelInArray = levelInArray.map((row, y) => { // mapping the level plan to create the actors on the level
            return row.map((character, x) => { // the two variables that will be used later for the creation of actors
                let type = levelCharactersDefined[character]; // type of character determined based on dictionnary, and different types of characters point to class
                if (typeof type == "string") {
                    return type; // returns the type/class if the character is a string
                }
...
var Coin = class Coin { // class used to create all the coins on the level (all coins must be collected to move on to the next level)
    ...
    get type() { // returns needed type for the level creation/css background fill
        return "coin";
    }
...
var levelCharactersDefined = { // every single character type is defined in this list, used by the var Level and the class Level to parse the intial level designs that are constructed in strings
...
var gameScreen = class gameScreen { // the gameScreen variable holds the table, in which all the actors and the background is created
    ...
    clear() {
        this.shownOnScreen.remove(); // when gameScreen.clear() is called, the div is removed from the document, temporarily, so that the game can be updated
    }
}
